...
class Solution {
    ...
    private static int getArea(int[][] grid,int i, int j){
        ...
        while(!q.isEmpty()) {
            ...
            Pair up = new Pair(p.i-1,j);
            Pair down = new Pair(p.i+1,j);
            ...
            if(up.isValid(n) && !visited[p.i-1][p.j] && grid[p.i -1][j] == 1) q.add(up);
            if(down.isValid(n) && !visited[p.i+1][p.j] && grid[p.i + 1][j] == 1) q.add(down);
            if(left.isValid(n) && !visited[p.i][p.j-1] && grid[p.i ][j-1] == 1) q.add(left);
            if(right.isValid(n) && !visited[p.i][p.j+1] && grid[p.i][j+1] == 1) q.add(right);
